...
export async function stageFunnel(scope?: {
  ...
  const where: any = {};
  if (scope?.teamId) where.assignedTeamId = scope.teamId;
...
export async function ratingsAggregation(scope?: {
  ...
  if (group === "stage") {
    const rows = await prisma.$queryRaw<
      Array<{ stage: string; avgRating: number | null; cnt: number }>
    >`SELECT l.stage as stage, AVG(f.rating) as "avgRating", COUNT(f.*) as cnt
       FROM "LeadFollowUp" f
