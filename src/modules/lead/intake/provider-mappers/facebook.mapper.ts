// src/modules/lead/provider-mappers/facebook.mapper.ts
import type { ProviderMappedInput } from "../types/type.js";
import { safeStr, pickFromFieldArray } from "./helpers.js";

export function mapFacebookLead(raw: any): ProviderMappedInput {
  ...
  try {
    ...
    const externalId = safeStr(
      value?.leadgen_id ?? value?.id ?? value?.lead_id ?? value?.leadId
    );
    const provider = "facebook_leads";

    // field_data: pick standard fields
    const fieldArr =
      value?.field_data ?? value?.fields ?? value?.form_response?.answers;
    ...
    const name =
      pickFromFieldArray(fieldArr, ["full_name", "full name", "name"]) ??
      safeStr(value?.name) ??
