// src/modules/lead/provider-mappers/google-ads.mapper.ts
 ...
 */
export function mapGoogleAdsLead(raw: any): ProviderMappedInput {
  try {
    ...
    const fieldsArray =
      payload?.fields ?? payload?.fieldData ?? payload?.leadFormFields ?? null;
    ...
    if (Array.isArray(fieldsArray)) {
      ...
      email = safeStr(
        payload?.email ?? payload?.emailAddress ?? payload?.contact?.email
      );
      phone = safeStr(
        payload?.phone ?? payload?.contact?.phone ?? payload?.phoneNumber
      );
      name = safeStr(payload?.name ?? payload?.contact?.name);
    }

    const externalId = safeStr(
      payload?.leadId ?? payload?.externalId ?? payload?.id
    );
