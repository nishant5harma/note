...
export function startTeamAssignmentWorker() {
  const concurrency = Math.max(
    1,
    Math.floor(
      parseInt(process.env.LEAD_ASSIGN_WORKER_CONCURRENCY ?? "4", 10) / 2
    )
  );
  ...
  const worker = new Worker(
    ...
    async (job) => {
      ...
      const startIdx =
        (((raw - 1) % members.length) + members.length) % members.length;
      ...
      for (let i = 0; i < members.length; i++) {
        ...
        try {
          ...
          if (!fresh || fresh.assignedToId) return;
          // Try DB claim
      ...
      const base = parseInt(
        process.env.LEAD_ASSIGN_BASE_DELAY_MS ?? "15000",
        10
      );
      ...
      const checkJobId = await enqueueAssignmentCheckJob(
        lead.id,
        assignmentId,
        { delayMs: delay, attempt: assignRow.attempt + 1 }
      );
