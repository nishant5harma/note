...
export function startAssignmentWorkers() {
  const concurrency = parseInt(
    process.env.LEAD_ASSIGN_WORKER_CONCURRENCY ?? "4",
    10
  );
  ...
  const worker = new Worker(
    ASSIGNMENT_QUEUE_NAME,
    async (job) => await assignmentJobHandler(job.data),
...
// If this module is executed directly (e.g. `npm run dev:worker`),
// start the workers immediately.
startAssignmentWorkers();
