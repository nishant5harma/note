...
import { findTeamForLead } from "./source-pool.service.js";
...
export async function assignmentJobHandler(data: {
  ...
  if (lead.assignedToId) {
    console.log(
      "lead already assigned by another worker; marking assignment row",
      lead.id
    );
  ...
  // If lead isn't associated to any team yet, resolve SourcePool mapping and persist assignedTeamId.
  // This ensures round-robin runs inside the correct team pool instead of the global fallback.
  if (!lead.assignedTeamId) {
    const teamId = await findTeamForLead(lead);
    if (teamId) {
  ...
  if (!candidate) {
    ...
    const maxDelay = parseInt(
      process.env.LEAD_ASSIGN_MAX_DELAY_MS ?? "600000",
      10
    );
    ...
    const maxAttempts = parseInt(
      process.env.LEAD_ASSIGN_MAX_ATTEMPTS ?? "10",
      10
    );
    ...
    console.log(
      "no candidate found; requeued with delay",
      delay,
      "attempt",
      nextAttempt
    );
  ...
  if (candidate.type === "user") {
    ...
    if (!acquiredLock) {
      ...
      const checkJobId = await enqueueAssignmentCheckJob(
        lead.id,
        assignRow.id,
        {
          delayMs: 2000,
  ...
  if (candidate.type === "team") {
    ...
    const job = await teamAssignmentQueue.add(
      "team-assign",
      { leadId: lead.id, assignmentId: assignRow.id, teamId },
      { jobId: `team:${assignRow.id}:${Date.now()}` }
    );
