 ...
 */
export async function enqueueAssignmentCheckJob(
  leadId: string,
  assignmentId: string,
  ...
  const job = await assignmentCheckQueue.add(
    "assignment-check",
    { leadId, assignmentId, attempt: opts?.attempt ?? 0 },
    { delay, jobId }
  );
 ...
 */
export async function enqueueAssignmentJob(
  leadId: string,
  assignmentId: string
): Promise<string> {
