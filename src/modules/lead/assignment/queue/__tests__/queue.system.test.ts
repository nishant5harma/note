...
const { enqueueAssignmentJob, enqueueAssignmentCheckJob } = await import(
  "../queue.js"
);
...
describe("assignment queue helpers (system)", () => {
  ...
  it("returns jobId on immediate assignment", async () => {
    ...
    expect(mockQueue.add).toHaveBeenCalledWith(
      "assign-lead",
      expect.objectContaining({ leadId: "l1", assignmentId: "a1", attempt: 0 }),
      expect.objectContaining({ jobId: "assign-a1" })
    );
  });
  ...
  it("respects BULLMQ_PREFIX for queue names", async () => {
    ...
    const { ASSIGNMENT_QUEUE_NAME } = await import(
      `../queue.js?update=${Date.now()}`
    );
