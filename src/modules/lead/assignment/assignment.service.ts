 ...
 */
export async function enqueueInitialAssignment(
  leadId: string,
  assignmentId: string
) {
  ...
  const newMeta = { ...(existingMeta as any), jobId };
...
export async function manualAssignLead(
  ...
  try {
    if (assignRow?.meta) {
      const meta = assignRow.meta as any;
      const toRemove: string[] = [];
      if (meta?.jobId) toRemove.push(meta.jobId);
      if (meta?.checkJobId) toRemove.push(meta.checkJobId);
      if (meta?.teamJobId) toRemove.push(meta.teamJobId);
    ...
    if (assignmentId) {
      await tx.leadAssignment.update({
        ...
        data: {
          ...
          assignedBy: actorId,
          method: "manual",
          attempt: { increment: 1 } as any,
          meta: {
            ...((assignRow?.meta as Record<string, any>) ?? {}),
