...
function parseOptionalDateIso(v: string | undefined) {
  if (!v) return undefined;
  const d = new Date(v);
  if (Number.isNaN(d.getTime())) throw new BadRequestError("Invalid date");
  return d;
}
...
async function listLeads(params: {
  ...
  if (createdAtGte || createdAtLte) {
    ...
    if (createdAtLte) where.createdAt.lte = createdAtLte;
  }
