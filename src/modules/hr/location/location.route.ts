...
LocationRouter.post(
  "/",
  requirePermission("location", "request"),
  LocationController.createLocationRequestHandler
);
...
LocationRouter.post(
  "/:id/respond",
  requireAuth,
  LocationController.respondLocationRequestHandler
);

// requester polls ephemeral result (only requester may read)
LocationRouter.get(
  "/:id/result",
  requirePermission("location", "read"), // keep read permission; controller checks requester
