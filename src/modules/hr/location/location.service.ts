...
import { pushToUser, emitToUser } from "@/modules/socket/socket.service.js";
import {
  BadRequestError,
  NotFoundError,
  UnauthorizedError,
...
import {
  ...
  sendPushToUser,
} from "@/modules/socket/util-socket/push.sender.js";

// src/modules/hr/location/location.service.ts

const LOCATION_REQUEST_TTL_SECONDS = parseInt(
  process.env.LOCATION_REQUEST_TTL_SECONDS ?? "60",
  10
);
const LOCATION_REQUEST_EXPIRY_SECONDS = parseInt(
  process.env.LOCATION_REQUEST_EXPIRY_SECONDS ?? "60",
  10
);
...
async function createLocationRequest(
  actor: AuthUser,
  targetUserId: string,
  expiresInSeconds?: number | undefined,
  note?: string | undefined
) {
  if (!actor) throw new UnauthorizedError("Actor required");
  if (!targetUserId) throw new BadRequestError("targetUserId required");

  await ensureUserExists(targetUserId);

  const expires = new Date(
    Date.now() + 1000 * (expiresInSeconds ?? LOCATION_REQUEST_EXPIRY_SECONDS)
  );
...
async function respondToLocationRequest(
  actor: AuthUser,
  requestId: string,
  payload: RespondLocationInput
) {
  const reqRow = await prisma.locationRequest.findUnique({
    where: { id: requestId },
  });
  if (!reqRow) throw new NotFoundError("Location request not found");
  ...
  await setWithTTL(
    key,
    { ...meta, responderId: actor.id },
    LOCATION_REQUEST_TTL_SECONDS
  );
...
async function getLocationRequestResult(actor: AuthUser, requestId: string) {
  const reqRow = await prisma.locationRequest.findUnique({
    where: { id: requestId },
  });
  if (!reqRow) throw new NotFoundError("Request not found");
  ...
  if (!data) {
    ...
    if (
      reqRow.expiresAt.getTime() < Date.now() &&
