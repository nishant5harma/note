...
async function createTeam(payload: {
  ...
  if (payload.teamLeadId) {
    ...
    if (!user) throw new BadRequestError("Lead user not found");
  }
...
async function listTeams() {
  return prisma.team.findMany({
    ...
    include: {
      ...
      members: { take: 5, include: { user: true } },
...
async function updateTeam(
  id: string,
  patch: { name?: string; leadId?: string | null }
) {
  if (patch.leadId) {
  ...
  if (patch.leadId !== undefined) data.teamLeadId = patch.leadId;
...
async function assignLead(teamId: string, teamLeadId: string) {
 ...
 */
async function isTeamLead(teamId: string, userId: string) {
...
async function assignMembers(
  teamId: string,
  userIds: string[],
  role: "MEMBER" | "ASSISTANT_LEAD" | "LEAD" = "MEMBER",
  actor?: AuthUser,
  actorHasTeamWrite = false
) {
  // validate team exists
  const team = await prisma.team.findUnique({ where: { id: teamId } });
  if (!team) throw new NotFoundError("Team not found");

  const results: { assigned: any[]; skipped: any[]; requests: any[] } = {
    assigned: [],
    skipped: [],
    requests: [],
  };
...
async function removeMember(
  ...
  if (!tm) throw new NotFoundError("Team member not found");
...
async function bulkRemoveMembers(
  ...
  for (const uid of userIds) {
    try {
      ...
      if ((res as any).removed) removed.push(uid);
      else if ((res as any).request) requests.push((res as any).request);
    } catch (err: any) {
...
async function createJoinRequest(
  ...
  const exists = await prisma.teamMember
    ...
    .catch(() => null);
  if (exists) throw new BadRequestError("User already a member of the team");
 ...
 */
async function listRequests(filter?: { status?: string; type?: string }) {
  const where: any = {};
...
async function respondToRequest(
  ...
  if (req.type === "transfer") {
    ...
    const result = await prisma.$transaction(async (tx) => {
      ...
      if (!old) throw new NotFoundError("Member not found in fromTeam");
      // delete old
