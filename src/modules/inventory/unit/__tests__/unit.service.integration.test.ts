...
/* 📦 IMPORT AFTER MOCKS                                              */
/* ------------------------------------------------------------------ */
const { createUnit, getUnit, listUnits, updateUnit, sellUnit, deleteUnit } =
  await import("../unit.service.js");
...
describe("Inventory Unit Service (integration)", () => {
  ...
  it("prevents updating SOLD unit", async () => {
    ...
    await expect(updateUnit(unit.id, { unitNumber: "X" })).rejects.toThrow(
      "already SOLD"
    );
  });
