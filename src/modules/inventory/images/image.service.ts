 ...
 */
async function removeStorageObject(storageKey?: string | null) {
...
export async function addImagesToListing(
  listingId: string,
  urlsOrObjs: Array<string | { url: string; storageKey?: string }>,
  actorId?: string
) {
...
export async function addImagesToUnit(
  unitId: string,
  urlsOrObjs: Array<string | { url: string; storageKey?: string }>,
  actorId?: string
) {
...
export async function removeUnitImage(imageId: string) {
...
export async function reorderListingImages(listingId: string, order: string[]) {
  // Validate images belong to listing
  const imgs = await prisma.listingImage.findMany({
    where: { listingId, id: { in: order } },
  });
  if (imgs.length !== order.length) throw new Error("invalid image ids");
  // update positions
...
export async function reorderUnitImages(unitId: string, order: string[]) {
