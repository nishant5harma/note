  ...
  for (const r of overdue) {
    ...
    try {
      ...
      await prisma.$transaction(async (tx) => {
        ...
        if (new Date(fresh.expiresAt) > now) return;
        ...
        try {
          ...
          if (ttlRow) {
            ...
            try {
              ...
              console.warn(
                `Reconciliation: failed to remove Bull job for ${r.id}`,
                jobErr
              );
            }
            ...
            try {
              ...
              console.warn(
                `Reconciliation: failed to delete TTL record for ${r.id}`,
                dbErr
              );
            }
          }
          ...
          console.warn(
            `Reconciliation: TTL cleanup error for reservation ${r.id}`,
            ttlErr
          );
        }
