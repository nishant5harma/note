 ...
 */
function normalizePhoneForStorage(p?: string | null) {
  if (!p) return null;
...
export async function uploadColdCallService(input: {
  ...
  const normalized = rows.map((r, idx) => {
    const phone = (r.phone ??
      ...
      r["phone number"] ??
      "") as string;
    const email = (r.email ?? r.mail ?? "") as string;
    const name = (r.name ?? r.fullname ?? r["full name"] ?? "") as string;
  ...
  return prisma.$transaction(async (tx) => {
    ...
    for (const r of normalized) {
      ...
      if (alreadyExists) duplicateCount++;
