...
const { tryAcquireLock, releaseLock, leadStatusLockKey, claimCapacityForUser, releaseCapacityClaim, } = await import("../redis.lock.js");
describe("Redis Lock Utils", () => {
    ...
    describe("tryAcquireLock", () => {
        it("should successfully acquire a lock when available", async () => {
            ...
            expect(mockRedisClient.set).toHaveBeenCalledWith("test-key", mockToken, "PX", 5000, "NX");
        });
        ...
        it("should use default TTL of 20000ms when not specified", async () => {
            ...
            expect(mockRedisClient.set).toHaveBeenCalledWith("test-key", mockToken, "PX", 20000, "NX");
        });
    ...
    describe("releaseLock", () => {
        it("should successfully release lock with matching token", async () => {
            ...
            expect(mockRedisClient.eval).toHaveBeenCalledWith(expect.stringContaining("redis.call"), 1, "test-key", "valid-token");
        });
    ...
    describe("claimCapacityForUser (deprecated)", () => {
        it("should increment claim counter for new user", async () => {
            ...
            expect(result).toBe(true);
            expect(mockRedisClient.incr).toHaveBeenCalledWith("lead:capacity:claims:user-123");
            expect(mockRedisClient.pexpire).toHaveBeenCalledWith("lead:capacity:claims:user-123", 30000);
        });
        ...
        it("should use default TTL of 60000ms when not specified", async () => {
            ...
            expect(mockRedisClient.pexpire).toHaveBeenCalledWith("lead:capacity:claims:user-789", 60000);
        });
    ...
    describe("releaseCapacityClaim (deprecated)", () => {
        it("should decrement claim counter", async () => {
            ...
            expect(mockRedisClient.decr).toHaveBeenCalledWith("lead:capacity:claims:user-123");
        ...
        it("should delete key when counter reaches zero", async () => {
            ...
            expect(mockRedisClient.del).toHaveBeenCalledWith("lead:capacity:claims:user-456");
        });
...
//# sourceMappingURL=redis.lock.unit.test.js.map