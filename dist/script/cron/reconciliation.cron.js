...
export function startReconciliationCron() {
    ...
    cron.schedule("*/5 * * * *", async () => {
        try {
            console.log(`[CRON] Running reservation reconciliation at ${new Date().toISOString()}`);
            const result = await runReservationReconciliation(300);
            // 300 = max items to process per batch (safe value)
            console.log(`[CRON] Reconciliation complete. Processed = ${result.processed}`);
        }
        catch (err) {
...
//# sourceMappingURL=reconciliation.cron.js.map