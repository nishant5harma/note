...
jest.unstable_mockModule("@/modules/inventory/reservation/worker/reconciliation.worker.js", () => ({
...
describe("reconciliation cron (system)", () => {
    ...
    it("executes the reconciliation worker when the cron triggers", async () => {
        ...
        const cronTask = mockSchedule.mock.calls[0]?.[1];
        if (typeof cronTask !== "function")
            throw new Error("Task not found");
        mockRunReconciliation.mockResolvedValue({ processed: 10 });
        await cronTask();
        expect(mockRunReconciliation).toHaveBeenCalledWith(300);
        expect(logSpy).toHaveBeenCalledWith(expect.stringContaining("Reconciliation complete"));
    });
    it("gracefully handles worker errors", async () => {
        ...
        expect(errorSpy).toHaveBeenCalledWith(expect.stringContaining("[CRON] Reconciliation error:"), expect.any(Error));
    });
});
//# sourceMappingURL=reconciliation.cron.system.test.js.map