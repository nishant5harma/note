...
describe("Team Service (Integration)", () => {
    ...
    describe("Workflow: Member Assignment", () => {
        it("should assign member directly if actor has teamWrite permission", async () => {
            const result = await TeamService.assignMembers(team.id, [targetUser.id], "MEMBER", adminUser, true);
        ...
        it("should create a Request if actor is Team Lead (no teamWrite)", async () => {
            const result = await TeamService.assignMembers(team.id, [targetUser.id], "MEMBER", leadUser, false);
    ...
    describe("Workflow: Request Approval & Rejection", () => {
        it("should perform membership change when an HR admin approves a transfer", async () => {
            ...
            const request = await TeamService.createTransferRequest(team.id, targetUser.id, teamB.id, leadUser);
            const response = (await TeamService.respondToRequest(request.id, true, adminUser));
        ...
        it("should mark request as rejected and NOT create membership if denied", async () => {
            ...
            const result = (await TeamService.respondToRequest(req.id, false, adminUser, "Not qualified"));
    ...
    describe("Security & Constraints", () => {
        it("should throw UnauthorizedError if a stranger tries to create a join request", async () => {
            ...
            await expect(TeamService.createJoinRequest(team.id, targetUser.id, stranger)).rejects.toThrow("Only team lead or target user can create join request");
        });
        it("should throw error if team still has members during deletion", async () => {
            await expect(TeamService.deleteTeam(team.id)).rejects.toThrow("Team has members; remove members before deleting");
        });
        it("should fail transfer if member is no longer in the source team", async () => {
            ...
            await expect(TeamService.respondToRequest(req.id, true, adminUser)).rejects.toThrow("Member not found in fromTeam");
        });
    });
...
//# sourceMappingURL=team.service.integration.test.js.map