 ...
 */
export function computeDedupeHash({ phone, email, source, externalId, }) {
    const normalizePhone = (p) => p ? p.replace(/[^\d+]/g, "") : "";
    const normalizeEmail = (e) => e ? e.trim().toLowerCase() : "";
    const parts = [
 ...
 */
export async function tryAcquireRedisGuard(key, ttlMs = 30_000) {
 ...
 */
export async function releaseRedisGuard(key) {
    const client = getRedisClient();
    await client.del(key);
}
//# sourceMappingURL=intake.util.js.map