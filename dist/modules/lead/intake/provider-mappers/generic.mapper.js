 ...
 */
export function mapGenericForm(raw, providerName = "generic_form") {
    try {
        ...
        for (const c of candidates) {
            if (!c || typeof c !== "object")
                continue;
            if (!email)
                email = safeStr(findByKeyCaseInsensitive(c, [
                    "email",
                    "emailAddress",
                    "email_address",
                    "e-mail",
            ...
            if (!phone)
                phone = safeStr(findByKeyCaseInsensitive(c, [
                    "phone",
                    "mobile",
                    "phone_number",
                    "contact",
            ...
            if (!name)
                name = safeStr(findByKeyCaseInsensitive(c, [
                    "name",
                    "fullName",
                    "full_name",
                    "first_name",
            ...
            if (!externalId)
                externalId = safeStr(findByKeyCaseInsensitive(c, [
                    "id",
                    "externalId",
                    "external_id",
                    "submissionId",
