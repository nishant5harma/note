...
/* ---------- Attendance service ---------- */
async function createCheckin(actor, payload) {
    // ensure user exists
...
async function listAttendance(filter) {
    ...
    if (filter.from || filter.to) {
        where.checkinAt = {};
        if (filter.from)
            where.checkinAt.gte = new Date(filter.from);
        if (filter.to)
            where.checkinAt.lte = new Date(filter.to);
    }
