...
import { registerDevice, unregisterDevice, listDevicesForUser, } from "../device.service.js";
describe("Device Service - Integration", () => {
    ...
    describe("unregisterDevice()", () => {
        ...
        it("should throw error for a device that belongs to another user", async () => {
            ...
            await expect(unregisterDevice({ userId: user.id, deviceId: "other-users-device" })).rejects.toThrow("Device not found");
        });
        it("should throw error for a non-existent device", async () => {
            await expect(unregisterDevice({ userId: user.id, deviceId: "non-existent" })).rejects.toThrow("Device not found");
        });
    });
    describe("listDevicesForUser()", () => {
        it("should return all devices belonging to the user", async () => {
            await deviceFactory.createList(3, { userId: user.id });
...
//# sourceMappingURL=device.service.integration.test.js.map