import { deviceRegisterSchema, deviceUnregisterSchema, } from "./validator/device.validator.js";
import { registerDevice, unregisterDevice, listDevicesForUser, } from "./device.service.js";
import { z } from "zod";
/**
 * POST /api/devices/register
 */
export async function registerDeviceHandler(req, res, next) {
    try {
 ...
 */
export async function unregisterDeviceHandler(req, res, next) {
    try {
 ...
 */
export async function listMyDevicesHandler(req, res, next) {
    try {
        if (!req.user)
            return res.status(401).json({ error: "unauthenticated" });
