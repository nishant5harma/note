 ...
 */
export async function parseXlsxBuffer(data) {
    ...
    for (let i = 0; i < dataRows.length; i++) {
        ...
        if (!rowArray || rowArray.every((v) => v == null || v === ""))
            continue;
        const obj = {};
        ...
        for (const k of Object.keys(obj)) {
            ...
            const v = obj[k];
            if (!normalized.phone && phoneKeys.includes(low))
                normalized.phone = v;
            if (!normalized.email && emailKeys.includes(low))
                normalized.email = v;
            if (!normalized.name && nameKeys.includes(low))
                normalized.name = v;
        }
...
//# sourceMappingURL=xlsx-parser.util.js.map