...
export async function setTeamQuota(teamId, period, metric, target) {
    // Upsert by unique (teamId, period, metric)
    const q = await prisma.coldCallQuota.upsert({
        where: { teamId_period_metric: { teamId, period, metric } },
        create: { teamId, period, metric, target },
        update: { target, active: true },
    });
    return q;
}
export async function getTeamQuotas(teamId) {
    return prisma.coldCallQuota.findMany({ where: { teamId } });
}
export async function getTeamQuotaProgress(teamId, period = "daily") {
