...
describe("pull.service – integration", () => {
    ...
    it("prevents lock refresh by non-owner", async () => {
        ...
        await expect(refreshLock(entry.id, intruder.id)).rejects.toThrow("not-lock-owner");
    });
    ...
    it("prevents non-owner from releasing active lock", async () => {
        ...
        await expect(releaseLock(entry.id, intruder.id, true)).rejects.toThrow("cannot-release-lock");
    });
...
//# sourceMappingURL=pull.service.integration.test.js.map