import { claimNextColdCallEntry, refreshLock, releaseLock, } from "./pull.service.js";
 ...
 */
export async function pullNextHandler(req, res, next) {
    try {
        ...
        if (!entry)
            return res.json({ ok: true, message: "no-available-entry" });
 ...
 */
export async function refreshLockHandler(req, res, next) {
    try {
        const userId = req.user?.id;
 ...
 */
export async function releaseLockHandler(req, res, next) {
    try {
        const userId = req.user?.id;
        const entryId = toStringSafe(req.params.id);
        if (!userId)
