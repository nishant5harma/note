 ...
 */
export async function claimNextColdCallEntry(userId, teamIds, lockMs = 2 * 60_000) {
 ...
 */
export async function refreshLock(entryId, userId, addMs = 2 * 60_000) {
 ...
 */
export async function releaseLock(entryId, userId, makePending = true) {
    return await prisma.$transaction(async (tx) => {
        ...
        const lockExpired = !entry.lockExpiresAt || new Date(entry.lockExpiresAt) < new Date();
        if (!isOwner && !lockExpired && userId !== "system") {
            throw new Error("cannot-release-lock");
        }
        const data = { lockUserId: null, lockExpiresAt: null };
        if (makePending)
            data.status = "pending";
        const updated = await tx.coldCallEntry.update({
            where: { id: entryId },
