 ...
 */
export async function getAgentPerformance(userId, from) {
 ...
 */
export async function getTeamPerformance(teamId, from) {
    const fromDate = from ?? startOfDay(new Date(Date.now() - 7 * 86400 * 1000));
    const rows = await prisma.coldCallAggregate.findMany({
 ...
 * getLeaderboard
 */
export async function getLeaderboard(topN = 10, metric = "conversions", sinceDays = 7) {
