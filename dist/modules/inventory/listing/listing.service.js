...
export async function listListings(filter = {}) {
    ...
    if (filter.sqftMin || filter.sqftMax) {
        ...
        if (filter.sqftMin !== undefined)
            where.sqft.gte = Number(filter.sqftMin);
        if (filter.sqftMax !== undefined)
            where.sqft.lte = Number(filter.sqftMax);
    }
 ...
 */
export async function updateListing(id, data, opts) {
 ...
 */
export async function closeListing(listingId, payload) {
    ...
    try {
        ...
        return prisma
            ...
            await cancelActiveReservationsForListing(listingId, "listing.closed", payload?.closedBy);
            // Mark listing as CLOSED
