...
import { addImagesToListing, addImagesToUnit, removeListingImage, reorderListingImages, } from "../image.service.js";
describe("Inventory Image Service (integration)", () => {
    it("adds images to listing with correct positions", async () => {
        ...
        if (!img1 || !img2)
            throw new Error("Expected 2 images");
        expect(img1.meta.position).toBe(0);
        expect(img2.meta.position).toBe(1);
    });
    it("adds images to unit", async () => {
        ...
        const img = images[0];
        if (!img)
            throw new Error("Expected unit image");
        expect(img.unitId).toBe(unit.id);
        expect(img.meta.position).toBe(0);
    });
    it("reorders listing images", async () => {
        ...
        if (!img0 || !img1 || !img2)
            throw new Error("Expected 3 images");
        ...
        const sorted = rows.sort((a, b) => (a.meta?.position ?? 0) - (b.meta?.position ?? 0));
    ...
    it("rejects invalid reorder ids", async () => {
        ...
        await expect(reorderListingImages(listing.id, ["invalid-id"])).rejects.toThrow("invalid image ids");
    });
});
//# sourceMappingURL=image.service.integration.test.js.map