 ...
 */
async function removeTtlJobIfExists(jobIdOrReservationId) {
    if (!jobIdOrReservationId)
        return;
    try {
...
export async function listUnits(filter = {}) {
    const page = typeof filter.page === "number" &&
    ...
    if (filter.bedrooms !== undefined)
        where.bedrooms = Number(filter.bedrooms);
 ...
 */
export async function updateUnit(id, data, opts) {
 ...
 */
export async function sellUnit(unitId, payload) {
    ...
    try {
        return prisma.$transaction(async (tx) => {
            ...
            const status = String(unit.status ?? "available").toUpperCase();
            if (status === "SOLD")
                throw new Error("unit already sold");
            ...
            for (const r of reservations) {
                ...
                try {
                    ...
                    console.warn("sellUnit: failed to remove TTL job for reservation", r.id, e);
                }
