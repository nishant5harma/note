...
const RESERVATION_EXPIRATION_QUEUE = process.env.BULLMQ_PREFIX
    ...
    : "reservation-expire";
export const reservationExpirationQueue = new Queue(RESERVATION_EXPIRATION_QUEUE, {
    connection,
 ...
 */
export async function enqueueReservationExpiration(reservationId, expiresAt) {
    const delay = Math.max(0, expiresAt.getTime() - Date.now());
    const jobId = `reservation-expire:${reservationId}`;
    const job = await reservationExpirationQueue.add("expire", { reservationId }, { jobId, delay });
    ...
    try {
        ...
        console.warn("enqueueReservationExpiration: failed to persist TTL job record", e);
...
export function startReservationWorker() {
    const concurrency = Math.max(1, parseInt(process.env.INVENTORY_WORKER_CONCURRENCY ?? "1", 10));
    const worker = new Worker(RESERVATION_EXPIRATION_QUEUE, async (job) => {
