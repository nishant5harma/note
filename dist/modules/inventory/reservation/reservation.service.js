...
import { enqueueReservationExpiration, reservationExpirationQueue, } from "./worker/reservation.worker.js";
import { ReservationStatus } from "@prisma/client";
import { tryAcquireLock, releaseLock } from "../../../utils/redis.lock.js";
const DEFAULT_TTL_MS = parseInt(process.env.DEFAULT_RESERVATION_TTL_MS ?? String(15 * 60 * 1000), 10); // 15 minutes default
 ...
 */
async function removeTtlJobIfExists(reservationId) {
    if (!reservationId)
        return;
    try {
        ...
        try {
            ...
            console.warn("removeTtlJobIfExists: failed to delete ReservationTTLJob record", e);
        }
    }
    catch (e) {
    ...
    try {
        ...
        if (unitId) {
            ...
            if (["blocked", "sold", "booked"].includes(st)) {
                throw new Error(`Unit is not available for reservation (status=${unit.status})`);
            }
            ...
            if (!["available"].includes(st)) {
                throw new Error(`Listing is not available for reservation (status=${listing.status})`);
            }
 ...
 */
export async function cancelReservation(id, cancelledBy) {
    ...
    const { acquiredLock, token } = await tryAcquireLock(resourceLockKey, LOCK_TTL);
    if (!acquiredLock || !token) {
        throw new Error("Resource busy. Please retry.");
    }
    try {
 ...
 */
export async function cancelActiveReservationsForUnit(unitId, reason = "inventory.sold", actor) {
 ...
 * - Returns count of cancelled reservations
 */
export async function cancelActiveReservationsForListing(listingId, reason = "inventory.closed", actor) {
