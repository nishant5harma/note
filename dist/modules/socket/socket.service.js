            ...
            try {
                ...
                if (authUserId) {
                    ...
                    if (suppliedUserId && suppliedUserId !== authUserId) {
                        console.warn(`socket.register: client supplied userId (${suppliedUserId}) different from authenticated user (${authUserId}) — ignoring supplied value.`);
                    }
                }
                else {
                ...
                if (!userId) {
                    console.warn("socket.register: no userId available (auth or payload)");
                    return;
                }
...
export function getIo() {
    if (!io) {
        throw new Error("Socket.IO not initialized. Call initSocketServer(server).");
    }
    return io;
}
/**
...
export async function pushToUser(userId, payload) {
    try {
        ...
        if (!ok) {
            console.log(`pushToUser: user ${userId} not connected; consider push.sender.ts fallback`);
            return false;
        }
        return true;
    }
